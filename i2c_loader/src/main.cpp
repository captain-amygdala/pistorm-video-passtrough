...
int main(int lArgCount, const char *pArgs[])
{
	...
	//	goto end;
        ...
        camgpio = gpiod_chip_get_line(chip, gpio);
	if (!camgpio) {
		perror("Get line failed\n");
	//	goto close_chip;
	}

	ret = gpiod_line_request_output(camgpio, CONSUMER, 0);
	if (ret < 0) {
	...
	try {
        ...
        gpiod_line_set_value(camgpio, 1);
        std::this_thread::sleep_for(1s);

	printf("Resetting FPGA...\n");

	gpiod_line_set_value(camgpio, 0);
	std::this_thread::sleep_for(1s);
